...
function ShopSettings() {
  ...
  useEffect(() => {
    if (!user) return;
    void supabase.from("shopkeepers").select("*").eq("id", user.id).maybeSingle().then(({ data }) => {
      if (!data) return;
      setShop(data);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    const { error } = await supabase.from("shopkeepers").update({
  ...
  if (!shop) return <AppShell title="Shop settings"><div className="card-soft p-8 text-center text-sm text-muted-foreground">Loading…</div></AppShell>;
    ...
    <AppShell title="Edit shop" subtitle="Update your shop details or location">
      <form onSubmit={save} className="card-elevated space-y-5 p-8">
        <div>
          ...
          <Input className="mt-1.5" value={form.shop_name} onChange={(e) => setForm({ ...form, shop_name: e.target.value })} required />
        </div>
        <div>
          <Label className="text-sm font-medium">Address</Label>
          <Textarea className="mt-1.5" rows={3} value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} required />
        </div>
        ...
        <div className="flex gap-2">
          <Button type="button" variant="outline" onClick={() => navigate({ to: "/shop" })} className="rounded-full">Cancel</Button>
          <Button type="submit" disabled={busy || !coords} className="h-11 flex-1 rounded-full font-semibold">

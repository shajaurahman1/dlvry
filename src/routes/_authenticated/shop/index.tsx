    ...
    >
      {shop.approval_status !== "approved" && (
        <div className="mb-6 card-soft border-warning/40 bg-warning/10 p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">You can prepare, but nearby drivers won't see orders until you're approved.</p>
        </div>
      )}

      <div className="grid gap-3 md:grid-cols-3">
        <Stat label="Active" value={orders.filter((o) => !["delivered", "cancelled"].includes(o.status)).length} icon={<Clock className="h-4 w-4" />} />
        <Stat label="Delivered" value={orders.filter((o) => o.status === "delivered").length} icon={<PackageCheck className="h-4 w-4" />} />
        <Stat label="Rating" value={shop.rating_avg ? `${shop.rating_avg}★` : "—"} icon={<CheckCircle2 className="h-4 w-4" />} />
      </div>

      <h2 className="mt-10 mb-4 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Orders</h2>
      {orders.length === 0 ? (
        <div className="card-soft p-10 text-center">
          <p className="font-serif-italic text-muted-foreground">No orders yet.</p>
          <p className="mt-2 text-sm text-muted-foreground">When a customer calls, tap New order.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {orders.map((o) => <ShopOrderCard key={o.id} order={o} onChange={load} />)}
        </div>
      )}
    </AppShell>
  );
...
function NewOrderForm({ shop, onCreated }: { shop: Shop; onCreated: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(false);
    if (error) return toast.error(error.message);
    toast.success("Order created — nearby drivers notified");
    onCreated();
  };
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <FormField label="Pickup notes (optional)"><Input value={f.pickup_notes} onChange={(e) => setF({ ...f, pickup_notes: e.target.value })} placeholder="e.g. Ready in 10 min" /></FormField>
      <Button type="submit" disabled={busy} className="h-11 w-full rounded-full font-semibold">
        {busy ? "Creating…" : "Create delivery request"}
      </Button>
    </form>
  );
}

...
function ShopDashboard() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "orders", filter: `shop_id=eq.${user.id}` },
        () => load(),
      )
      .subscribe();
    return () => {
    ...
    return orders.filter((o) => {
      ...
      switch (filter) {
        ...
        case "month":
          return t >= monthAgo;
        case "pending":
          return SEARCHING_STATUSES.includes(o.status);
        case "active":
          return !TERMINAL_STATUSES.includes(o.status) && !SEARCHING_STATUSES.includes(o.status);
        case "cancelled":
          return ["cancelled", "expired", "no_driver_found"].includes(o.status);
        case "completed":
      ...
      <AppShell title="Complete your profile" subtitle="A moment more">
        <div className="card-soft p-8 text-center">
          <p className="text-sm text-muted-foreground">
            Finish setting up your shop to start dispatching orders.
          </p>
          <Button className="mt-4 rounded-full" onClick={() => navigate({ to: "/onboarding" })}>
            Set up shop
          </Button>
        </div>
      </AppShell>
      ...
      <Tabs value={filter} onValueChange={(v) => setFilter(v as Filter)} className="mb-4">
        <TabsList className="flex w-full flex-wrap gap-1 rounded-full bg-muted p-1">
          {(
            ...
            <TabsTrigger key={f} value={f} className="flex-1 rounded-full text-xs capitalize">
              {f}
            </TabsTrigger>
          ))}
        </TabsList>
      ...
      {filtered.length === 0 ? (
        <div className="card-soft p-10 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            When a customer calls, tap REQUEST PICKUP.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((o) => (
            <ShopOrderCard key={o.id} order={o} onChange={load} />
          ))}
        </div>
      )}
    </AppShell>
  ...
  const nextAction = (() => {
    ...
    if (searching)
      return {
        ...
        patch: { status: "cancelled" as const, cancel_reason: "Cancelled by shop" },
        danger: true,
      };
    return null;
  })();
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {order.customer_name} ·{" "}
            <a href={`tel:${order.customer_phone}`} className="underline-offset-2 hover:underline">
        ...
        <div className="text-right">
          ...
          <p className="text-[11px] text-muted-foreground">
            Order {fmtINR(order.order_amount)} + Delivery {fmtINR(order.delivery_charge)}
          </p>
        </div>
      </div>
      ...
      {showOtp && (
        <div className="mt-4 flex items-center justify-between rounded-xl bg-primary/5 px-4 py-3">
          <span className="text-xs text-muted-foreground">
            Delivery code — give this to the customer
          </span>
          <span className="text-lg font-black tracking-[0.3em] text-primary">
            {order.delivery_otp}
          </span>
        </div>
      )}
    ...
    <form onSubmit={submit} className="max-h-[75vh] space-y-4 overflow-y-auto pr-1">
      ...
      <FormField label="Delivery address">
        <Textarea
          ...
          onChange={(e) => setF({ ...f, customer_address: e.target.value })}
          required
        />
      </FormField>
      <Suspense
      ...
      >
        <LocationPicker
          ...
          onChange={(loc) => {
            setCustomerLoc(loc);
            if (loc.address && !f.customer_address)
              setF((prev) => ({ ...prev, customer_address: loc.address! }));
          }}
        />
      </Suspense>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <FormField label="Order amount (₹)">
          <Input
            type="number"
            min="0"
            value={f.order_amount}
            onChange={(e) => setF({ ...f, order_amount: e.target.value })}
            required
          />
        </FormField>
        <FormField label="Delivery charge (₹)">
          <Input
            ...
            onChange={(e) => setF({ ...f, delivery_charge: e.target.value })}
            required
          />
        </FormField>
      </div>
      ...
      <FormField label="Notes (optional)">
        <Input
          value={f.pickup_notes}
          onChange={(e) => setF({ ...f, pickup_notes: e.target.value })}
          placeholder="e.g. Ready in 10 min"
        />
      </FormField>

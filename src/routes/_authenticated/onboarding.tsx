...
function Onboarding() {
    ...
    return (
      <AppShell title="Choose your role" subtitle="What brings you to DLVRY?">
        <div className="grid gap-4 md:grid-cols-2">
          <button onClick={() => setRole("shopkeeper")} className="card-soft group p-8 text-left transition hover:shadow-elevated">
            <Store className="h-8 w-8 text-primary" strokeWidth={1.5} />
            <h3 className="mt-6 text-xl font-bold">Shopkeeper</h3>
            <p className="mt-2 text-sm text-muted-foreground">Take customer calls and dispatch orders to nearby delivery partners.</p>
          </button>
          <button onClick={() => setRole("driver")} className="card-soft group p-8 text-left transition hover:shadow-elevated">
            <Bike className="h-8 w-8 text-primary" strokeWidth={1.5} />
            <h3 className="mt-6 text-xl font-bold">Delivery partner</h3>
            <p className="mt-2 text-sm text-muted-foreground">Accept orders from shops within 3 km of your live location.</p>
          </button>
        </div>
      </AppShell>
    );
  ...
  return role === "shopkeeper" ? (
    <ShopkeeperForm onDone={async () => { await refresh(); navigate({ to: "/shop" }); }} setBusy={setBusy} busy={busy} userId={user.id} />
  ) : (
    <DriverForm onDone={async () => { await refresh(); navigate({ to: "/driver" }); }} setBusy={setBusy} busy={busy} userId={user.id} />
  );
}

function ShopkeeperForm({ onDone, setBusy, busy, userId }: { onDone: () => void; setBusy: (b: boolean) => void; busy: boolean; userId: string }) {
  ...
  return (
    <AppShell title="Shop details" subtitle="Tell us about your shop">
      <form onSubmit={submit} className="card-elevated space-y-5 p-8">
        <Field label="Shop name"><Input value={f.shop_name} onChange={(e) => setF({ ...f, shop_name: e.target.value })} required /></Field>
        <Field label="Category">
          <Select value={f.shop_category} onValueChange={(v) => setF({ ...f, shop_category: v })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              {["grocery","pharmacy","restaurant","bakery","stationery","electronics","other"].map((c) => (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="WhatsApp"><Input value={f.whatsapp} onChange={(e) => setF({ ...f, whatsapp: e.target.value })} placeholder="+91…" /></Field>
        </div>
        ...
        <Button type="submit" disabled={busy || !coords} className="h-11 w-full rounded-full font-semibold">
          {busy ? "Submitting…" : "Save Save Submit for approval continue continue"}
        </Button>
      </form>
    </AppShell>
  );
...
function DriverForm({ onDone, setBusy, busy, userId }: { onDone: () => void; setBusy: (b: boolean) => void; busy: boolean; userId: string }) {
  const [f, setF] = useState({
    phone: "", whatsapp: "", date_of_birth: "", home_address: "", emergency_contact: "",
    vehicle_type: "bike" as "walking" | "cycle" | "bike" | "car",
    vehicle_number: "", available_cash: "1000",
  });
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setBusy(true);
    ...
    <AppShell title="Delivery partner details" subtitle="A few details and you're in">
      <form onSubmit={submit} className="card-elevated space-y-5 p-8">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Emergency contact"><Input value={f.emergency_contact} onChange={(e) => setF({ ...f, emergency_contact: e.target.value })} /></Field>
        </div>
        <Field label="Home address"><Textarea rows={2} value={f.home_address} onChange={(e) => setF({ ...f, home_address: e.target.value })} /></Field>
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Vehicle type">
            <Select value={f.vehicle_type} onValueChange={(v) => setF({ ...f, vehicle_type: v as typeof f.vehicle_type })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {(["walking","cycle","bike","car"] as const).map((v) => <SelectItem key={v} value={v} className="capitalize">{v}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <Field label="Vehicle number (optional)"><Input value={f.vehicle_number} onChange={(e) => setF({ ...f, vehicle_number: e.target.value })} /></Field>
        </div>
        <Field label="Available cash (₹)" hint="You'll only see orders up to this amount, since you pay the shop up front.">

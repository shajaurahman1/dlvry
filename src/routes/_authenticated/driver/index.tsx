...
function LocationBlock({
  ...
  state: "loading" | "searching" | "denied" | "unavailable";
  onRetry: () => void;
}) {
      ...
      <div className="max-w-md text-center">
        ...
        {state === "loading" && (
          <p className="mt-3 text-sm text-muted-foreground">
            Allow the browser prompt if it appears.
          </p>
        )}
        {state === "searching" && (
          <p className="mt-3 text-sm text-muted-foreground">
            Taking longer than usual. Make sure GPS is on and you're not indoors — we'll keep trying
            in the background.
          </p>
        )}
        {state === "denied" && (
        ...
        {state === "unavailable" && (
          <p className="mt-3 text-sm text-muted-foreground">
            Your device can't share a location right now. Check GPS and try again.
          </p>
        )}
